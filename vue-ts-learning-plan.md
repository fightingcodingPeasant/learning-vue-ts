...
| 5️⃣ | Composable 逻辑复用 | 2h | 🔲 未开始 |
| 6️⃣ | 响应式系统类型推断 | 2h | 🔲 未开始 |
...
- 防抖搜索实战（watch + setTimeout）

### 练习题

- [x] WatchDemo.vue — watch 监听数字变化
- [x] TextWatchDemo.vue — watch 监听字符串 + 条件变色
- [x] WatchEffectDemo.vue — watchEffect 自动追踪多个依赖
- [x] LifecycleDemo.vue — 生命周期 + 防抖搜索综合实战

### 复盘问题

- [x] computed 和 watch 怎么选？→ 要值用 computed，做事用 watch
- [x] watchEffect 和 watch 有什么区别？→ watch 手动指定监听源且能拿旧值，watchEffect 自动追踪
- [x] 为什么搜索要用防抖？→ 等用户停下来 500ms 再发请求，避免频繁触发
- [x] onUnmounted 不清理定时器会怎样？→ 内存泄漏，页面卡死
...
> **防抖搜索 = 实用技巧** —— 每次输入先 clearTimeout，再 setTimeout 500ms。这样快速打字时不会频繁触发，只有停下来才发一次请求。
...
*最后更新：2026-07-18 — Session 4 完成（Watchers & 生命周期）*
